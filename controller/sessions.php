...
if (array_key_exists('SES_ID', $_GET)) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        $returnData = $sessionDB->obtenerPorId($SES_ID, $SES_ID = $_GET['USE_ID']);
        $rowCount = count($returnData);
        if ($rowCount === 0) {
            $response->sendParams(false, 401, 'No se encontró la sesión');
        }
        $response->sendParams(true, 200, 'Sesión', $returnData);
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
        ...
        if (!($jsonData->SES_REFTOK) || ($jsonData->SES_REFTOK) < 1) { //Se verifican campos obligatorios
        ...
        try {
            ...
            //$response->sendParams(false, 401, $returnData['USER'][0]);
            ...
            $SES_ACCTOKEXP = date('d/m/Y H:i', time() + 60); //1 minuto
            $SES_REFTOKEXP = date('d/m/Y H:i', time() + 1209600); //1 dias
            ...
            $lastSession = $sessionDB->obtenerPorId($SES_ID, $user['USE_ID']);
    ...
    if (!($jsonData->USE_USERNAME) || !($jsonData->USE_PASSWORD)) { //Se verifican campos obligatorios
        $messages = array();
    ...
    try {
        ...
        $SES_ACCTOKEXP = date('d/m/Y H:i', time() + 30);
    ...
    try {
        ...
        $lastSession = $sessionDB->obtenerPorId($lastSessionId, $foundUser['USE_ID']);
        $rowCount = count($lastSession);
