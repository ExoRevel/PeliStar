...
switch ($_SERVER['REQUEST_METHOD']) {
    ...
    case 'GET':
        if (isset($_GET['MOVIE_ID'])) {
            if (isset($_GET['MOVIE_ID']) && isset($_GET['GENERO_ID'])) {
                try {

                    $movie_generoDB = new Movie_generoDB($database);
                    $movie_genero = new Movies_Generos($_GET['GENERO_ID'], $_GET['MOVIE_ID']);

                    $rowCount = $movie_generoDB->obtenerMovieGenero($movie_genero);
                    $rowCount = count($rowCount);
                    if ($rowCount !== 0) {
                        $response->sendParams(false, 201, 'ESTE GENERO YA SE ENCUENTRA REGISTRADO A LA PELICULA SELECCIONADA');
                    }
                } catch (Movies_GenerosException $ex) {
                    $response->sendParams(false, 400, $ex->getMessage());
                } catch (PDOException $ex) {
                ...
                try {
                    ...
                    $data = $movie_Generos->obtenerPorMovieId($_GET['MOVIE_ID']);
                    $rowCount = count($data);
