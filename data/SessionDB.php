<?php
    ...
    class SessionDB{
        ...
        public function reemplazar($session, $USE_ID){     
            ...
            $query->bindParam(1, $session->getAcctok(), PDO::PARAM_STR);
            $query->bindParam(2, $session->getAcctokexp(), PDO::PARAM_STR);    
            $query->bindParam(3, $session->getReftok(), PDO::PARAM_STR);
            $query->bindParam(4, $session->getReftokexp(), PDO::PARAM_STR);    
            $query->bindParam(5, $session->getId(), PDO::PARAM_INT);       
        ...
        public function obtenerPorId($SES_ID){
            $query = $this->database->prepare('SELECT SES_ID, USE_ID, SES_ACCTOK, DATE_FORMAT(SES_ACCTOKEXP, "%d/%m/%Y %H:%i") AS SES_ACCTOKEXP, SES_REFTOK, DATE_FORMAT(SES_REFTOKEXP, "%d/%m/%Y %H:%i") AS SES_REFTOKEXP  FROM SESSIONS WHERE SES_ID = ?');
